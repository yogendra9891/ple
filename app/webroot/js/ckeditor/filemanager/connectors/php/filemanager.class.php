...
// 		}
		...
		$current_path = $_SERVER['DOCUMENT_ROOT'].'/ple_vers7/app/webroot/img/ckeditor/';
		if(!$this->isValidPath($current_path)) {
			$this->error("No way.");
		}
		...
		$image_url = 'http://'.$_SERVER['HTTP_HOST'].'/ple_vers7/img/ckeditor/'.$_FILES['newfile']['name'];
		echo "Done";
       // echo 'http://'.$_SERVER['HTTP_HOST'].'/ple_design/img/ckeditor/'.$_FILES['newfile']['name']."<br />";
        echo "<script>parent.setCkUrl('$image_url')</script>";
       // echo "<span><input type='button' value='Select' onClick=setCkUrl('$image_url')></span>";
